...
if (!function_exists('getPro'))   {
  function getPro()  {
	...
	while($row_product=mysqli_fetch_array($run_products)){
		...
		echo"
			<div class='col-md-6 col-lg-3 border rounded shadow-sm mb-3'>
		<div class='product'>
							<a href='details.php?pro_id=$pro_id'>
								<img src='admin_area/product_images/$pro_image' class='img-fluid'>
							</a>
							<div class='text best_seller_products'>
	...
	if (isset($_GET['cat'])) {
		...
		while ($row_products=mysqli_fetch_array($run_products)) {
			...
			$pro_title=$row_products['product_title'];
 								...
 								<p class='price'>INR $pro_price</p>
 								<p class='buttons'>
 								<a href='details.php?pro_id=$pro_id,cat_id=$cat_id' class='btn details'>View-Details</a>
...
if (!function_exists('getPCats'))   {
  function getPCats()  {
	...
	while($row_p_cats=mysqli_fetch_array($run_p_cats)){
		...
		echo " <li class='list-group-item nav-item'><a href='shop.php?p_cat=$p_cat_id' class='nav-link'>$p_cat_title</a></li>";
}
}
}
}
	...
	if (isset($_GET['p_cat'])) {
		...
		while ($row_products=mysqli_fetch_array($run_products)) {
			...
			$pro_title=$row_products['product_title'];
 								...
 								$pro_image=$row_products['product_image'];
 								echo "
 								<div class='col-md-4 col-sm-6 border rounded shadow-sm'>
 								<div class='product'>
 								<a href='details.php?pro_id=$pro_id'><img src='admin_area/product_images/$pro_image' class='img-fluid'>
 								</a>
 								<div class='text'>
